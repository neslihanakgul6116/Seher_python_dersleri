...
st.set_page_config(page_title="Akıllı Belge Tarayıcı", page_icon="📄", layout="wide")
...
def process_document(image):
    ...
    tresh = cv2.adaptiveThreshold(warped_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
...
st.markdown("YOLO kullanmadan saf matematiksel geometri ve kontur analiziyle belgelerinizi tarayın, PDF olarak kaydedin.")

menu = st.sidebar.selectbox("Mod Seçin", ["Fotoğraf Yükle", "Kamera / Video Akışı"])

if menu == "Fotoğraf Yükle":
    st.subheader("📷 Fotoğraf Üzerinden Belge Tarama")
    uploaded_file = st.file_uploader("Bir belge fotoğrafı seçin...", type=["jpg", "jpeg", "png"])
    ...
    if uploaded_file is not None:
        ...
        with col1:
            st.image(cv2.cvtColor(opencv_image, cv2.COLOR_BGR2RGB), caption="Orijinal Görüntü", use_column_width=True)
        ...
        if st.button("Belgeyi Tara ve Düzelt"):
            ...
            with col2:
                st.image(scanned_image, caption="Taranmış Belge (Fotokopi Modu)", use_column_width=True, clamp=True)
    ...
    if run_camera:
        ...
        while run_camera:
            ...
            if capture_button:
                ...
                st.image(scanned_image, caption="Yakalanan ve Taranan Belge", clamp=True)
                pdf_path = save_as_pdf(scanned_image)
                with open(pdf_path, "rb") as pdf_file:
                    st.download_button(
